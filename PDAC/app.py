...
class Config:
    ...
    question = "我今天该穿什么？"
    data_path = "D:\\服装搭配大模型\\代码\\PDAC\\Data"
    persist_dir = "./chroma_db_test"
    embedding_model_path = "D:\\服装搭配大模型\\代码\\bge-m3"
    max_new_tokens = 8192
...
def load_vector_database(persist_dir: str) -> ChromaVectorStore:
    ...
    if os.path.exists(persist_dir):
        ...
        chroma_collection = chroma_client.get_collection("PDAC")
    else:
        print(f"创建新的向量数据库: {persist_dir}")
        chroma_client = chromadb.PersistentClient(path=persist_dir)
        chroma_collection = chroma_client.create_collection("PDAC")
...
if __name__ == "__main__":
    ...
    # 加载向量数据库
    vector_store = load_vector_database(persist_dir=config.persist_dir)
    message = "The motif composed of one macrophage, one cancer-associated fibroblast (CAF), and two plasma cells appears in large quantities in tissues from Pancreatic Ductal Adenocarcinoma patients untreated with radiotherapy and chemotherapy, but rarely in tissues from Pancreatic Ductal Adenocarcinoma patients treated with radiotherapy and chemotherapy. Please analyze the function of this motif"
